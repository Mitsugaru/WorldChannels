	...
	public void chatEvent(final PlayerChatEvent event)
	{
		...
		try
		{
			info.put(Field.GROUP,
					WorldChannels.chat.getPlayerGroups(event.getPlayer())[0]);
		}
		catch (ArrayIndexOutOfBoundsException a)
		{
			// IGNORE
		}
		info.put(Field.PREFIX, WorldChannels.chat.getPlayerPrefix(worldName,
				event.getPlayer().getName()));
		info.put(Field.SUFFIX, WorldChannels.chat.getPlayerSuffix(worldName,
				event.getPlayer().getName()));
		...
		if (plugin.getPermissionsHandler().checkPermission(event.getPlayer(),
